// =============================================================================
// conv_fixed_float_perf_debug.c
// CLEAN_SIMPLE_POINT_TEST_V3_WITH_PHASE7D_PERF_COUNTERS
//
// ELEN90093 CNN Accelerator - Simple Fixed16 + Float16 5x5 Point Check
// with hardware performance counter readback through FUNCT_DEBUG.
//
// Tests:
...
#define PERF_TOTAL_CYCLES          0
#define PERF_LOAD_REQS             1
#define PERF_STORE_REQS            2
#define PERF_REUSE_TILES           3
#define PERF_FULL_LOAD_TILES       4
#define PERF_COMPUTE_WAIT_CYCLES   5
#define PERF_OUTPUTQ_FULL_STALLS   6
#define PERF_MEM_BUSY_CYCLES       7
#define PERF_TILES_ISSUED          8
#define PERF_TILES_LOADED          9
#define PERF_TILES_COMPUTED        10
#define PERF_TILES_STORED          11
...
typedef struct {
    ...
    uint64_t load_reqs;
    uint64_t store_reqs;
    uint64_t reuse_tiles;
    uint64_t full_load_tiles;
    uint64_t compute_wait_cycles;
    uint64_t outputq_full_stalls;
    uint64_t mem_busy_cycles;
    uint64_t tiles_issued;
    uint64_t tiles_loaded;
    uint64_t tiles_computed;
    uint64_t tiles_stored;
} PerfCounters;
...
static inline uint64_t conv_debug(uint64_t counter_id) {
    uint64_t ret;
    ROCC_INSTRUCTION_DSS(ROCC_X, ret, counter_id, 0, FUNCT_DEBUG);
    return ret;
}

static void read_perf_counters(PerfCounters *p) {
    p->total_cycles = conv_debug(PERF_TOTAL_CYCLES);
    p->load_reqs = conv_debug(PERF_LOAD_REQS);
    p->store_reqs = conv_debug(PERF_STORE_REQS);
    p->reuse_tiles = conv_debug(PERF_REUSE_TILES);
    p->full_load_tiles = conv_debug(PERF_FULL_LOAD_TILES);
    p->compute_wait_cycles = conv_debug(PERF_COMPUTE_WAIT_CYCLES);
    p->outputq_full_stalls = conv_debug(PERF_OUTPUTQ_FULL_STALLS);
    p->mem_busy_cycles = conv_debug(PERF_MEM_BUSY_CYCLES);
    p->tiles_issued = conv_debug(PERF_TILES_ISSUED);
    p->tiles_loaded = conv_debug(PERF_TILES_LOADED);
    p->tiles_computed = conv_debug(PERF_TILES_COMPUTED);
    p->tiles_stored = conv_debug(PERF_TILES_STORED);
}

static void print_perf_counters(const char *name, const PerfCounters *p) {
    printf("[%s] Phase 7D hardware counters:\n", name);
    printf("  total sCompute cycles       : %lu\n", (unsigned long)p->total_cycles);
    printf("  load memory requests        : %lu\n", (unsigned long)p->load_reqs);
    printf("  store memory requests       : %lu\n", (unsigned long)p->store_reqs);
    printf("  reuse tiles                 : %lu\n", (unsigned long)p->reuse_tiles);
    printf("  full-load tiles             : %lu\n", (unsigned long)p->full_load_tiles);
    printf("  compute wait cycles         : %lu\n", (unsigned long)p->compute_wait_cycles);
    printf("  outputQ full stall cycles   : %lu\n", (unsigned long)p->outputq_full_stalls);
    printf("  memory busy cycles          : %lu\n", (unsigned long)p->mem_busy_cycles);
    printf("  tiles issued/loaded/comp/st : %lu / %lu / %lu / %lu\n",
           (unsigned long)p->tiles_issued,
           (unsigned long)p->tiles_loaded,
           (unsigned long)p->tiles_computed,
           (unsigned long)p->tiles_stored);
}
...
static int run_hw_5x5(uint64_t data_type,
                      const char *name,
                      uint64_t *cycles,
                      PerfCounters *perf) {
    uint64_t ret;
    ...
    *cycles = t1 - t0;
    ...
    // These reads happen after the timed region, so debug readback overhead is
    // not included in the printed HW cycle count.
    read_perf_counters(perf);
...
static int run_fixed_5x5(void) {
    ...
    printf("\n=== Fixed16 5x5 CLEAN V3 simple point check + perf counters ===\n");
    init_fixed_case();

    if (!run_hw_5x5(DATA_FIXED16, "Fixed16_5x5", &cycles, &perf)) {
        return 0;
    }

    printf("[Fixed16_5x5] HW cycles: %lu\n", (unsigned long)cycles);
    print_perf_counters("Fixed16_5x5", &perf);
...
static int run_float_5x5(void) {
    ...
    printf("\n=== Float16 5x5 CLEAN V3 simple point check + perf counters ===\n");
    init_float_case();

    if (!run_hw_5x5(DATA_FLOAT16, "Float16_5x5", &cycles, &perf)) {
        return 0;
    }

    printf("[Float16_5x5] HW cycles: %lu\n", (unsigned long)cycles);
    print_perf_counters("Float16_5x5", &perf);
    return check_points_float();
}

int main(void) {
    printf("=== CLEAN_SIMPLE_POINT_TEST_V3_WITH_PHASE7D_PERF_COUNTERS ===\n");
    ...
    printf("\n=== Final Result CLEAN V3 + Perf Counters ===\n");

...
// Keep buffers static and aligned.
// The accelerator STORE writes 64-bit chunks, so output must be 8-byte aligned.
// 64-byte alignment is safer for cache-line behavior.
...
// CONFIG: rs1 = kernel_size, rs2 = output address, rd = success/error
...
// LOAD: rs1 = input address, rs2 = kernel address, rd = success/error
...
// COMPUTE: no source registers, rd = success/error
...
// STORE: output address was already configured in CONFIG, rd = success/error
...
{
    if (((uint64_t)input_buf  & 7ULL) != 0) return 0;
    if (((uint64_t)kernel_buf & 7ULL) != 0) return 0;
    if (((uint64_t)hw_output  & 7ULL) != 0) return 0;
    return 1;
}
...
static void init_data(void)
{
    ...
    for (int i = 0; i < INPUT_ELEMS / 4; i++) {
        ...
        // Use 64-bit stores to avoid CPU-generated partial stores.
...
int main(void)
{
    ...
    // Make CPU initialization stores visible before RoCC LOAD.
    full_fence();
    ...
    // Make accelerator STORE visible before CPU checks output.
    full_fence();
